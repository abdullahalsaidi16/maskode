...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, Maskode does not support MCP authentication yet.`
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`maskode models\` to list available models`,
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`maskode auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
