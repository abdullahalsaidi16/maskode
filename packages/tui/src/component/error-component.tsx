      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>Maskode {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  ...
  const url = new URL("https://github.com/abdullahalsaidi16/maskode/issues/new")
  url.searchParams.set("title", `TUI crash: ${message}`)
  url.searchParams.set("maskode-version", InstallationVersion)
  url.searchParams.set("os", describeOS())
  url.searchParams.set("terminal", describeTerminal())
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the Maskode crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n… (truncated)"
  const head = `The Maskode TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
